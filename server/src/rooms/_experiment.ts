...
// Arena configuration
export const ARENA_CENTER = {x: 400, y: 300};
...
type ZoneDefinitions = Record<string, ZoneDefinition>;

const ZONE_DEFINITIONS: ZoneDefinitions = {
	Top: {
		id: 'Top', x: 400, y: 150, radius: 100,
	},
	Right: {
		id: 'Right', x: 650, y: 300, radius: 100,
	},
	Bottom: {
		id: 'Bottom', x: 400, y: 450, radius: 100,
	},
	Left: {
		id: 'Left', x: 150, y: 300, radius: 100,
	},
};

export class ExperimentRoom extends Room<ExperimentRoomState> {
	// Declare class properties
	private readonly maxRounds = 9;
	private readonly roundDuration: number = 60 * 1000;
	private readonly roundTimer: NodeJS.Timeout | undefined = null;
	private readonly logTimer: NodeJS.Timeout | undefined = null;
	// Private dataLogger: DataLogger;
	private readonly logInterval = 1000;
	private readonly playersInZones: Record<string, Set<string>> = {};
	private adminSessionId: string | undefined = null;

	onCreate(options: any) {
		console.log('ExperimentRoom created!', options);
		// This.setState(new ExperimentRoomState());

		// this.dataLogger = new DataLogger('experiment_data_colyseus.jsonl');

		// Initialize Zones in state with type safety
		// Object.entries(ZONE_DEFINITIONS).forEach(([zoneKey, zd]) => {
		//     this.state.zones.set(zoneKey, new ZoneState(zd.id, zd.x, zd.y, zd.radius));
		//     this.playersInZones[zoneKey] = new Set();
		// });

		// // Register message handlers with proper types
		// this.onMessage("playerMovement", (client: Client, message: { x: number, y: number }) => {
		...
		this.onMessage('setReady', (client, message) => {
			...
			if (player && this.state.phase === PHASES.LOBBY) {
				player.ready = message.isReady === undefined ? !player.ready : message.isReady;
	...
	startInstructions() {
		...
		}, 10_000); // Show instructions for 10 seconds
	}
	...
	assignRoles() {
		...
		for (const [index, id] of shuffledIds.entries()) {
			const player = this.state.players.get(id);
			if (player) {
				player.role = index < informedCount ? 'informed' : 'uninformed';
			}
		}
	...
	//         debrief: "This was an experiment on collective decision-making..."
	//     };
	...
	//             this.broadcast("playerEmoted", { id: client.sessionId, emote: emote }, { except: client });
	...
	//     if (this.state.phase !== PHASES.ROUND_ACTIVE && !isFinalLog) return;
